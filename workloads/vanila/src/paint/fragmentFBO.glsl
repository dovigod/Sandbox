...
void main(){
  ...
  //fbm noise
  // vec2 disp = aspect * (fbm(vUv * 24.0, 8)) * 0.009; // speed of color spread
  ...
  waterColor = blendDarken(prev.rgb, floodColor * (1.00004), 0.3);
  waterColor = blendDarken(prev.rgb, floodColor*(1. + 0.02), 0.7);

  vec3 finalColor = blendDarken(waterColor, mouseColor, 1.);
  ...
  // gl_FragColor = vec4(lerp(finalColor,bgColor, uTime / 1000.) , 1.0);

  gl_FragColor = vec4(
// min(bgColor, finalColor ),1.0);
...
min(bgColor * 1., finalColor * (1. + 0.005) + 0.0006869),1.0);
  ...
  // );
    ...
    gl_FragColor = vec4(
    blend(bgColor, finalColor, uFrame / 360.)
    ,1.0
  );
