...
void main(){
  ...
  float movement = info.y * 0.3 * mix(0.5,1., circularForce);
  float angle = atan(pos.y, pos.x) - movement;


  float targetRadius = mix(info.x, 1.8, 0.5 + 0.45*sin(angle * 2. + time * 1.));
  ...
  pos.xy += curl(pos.xyz * 2., time* 0.3, 0.1).xy * 0.007;
